    ...
    for person in os.listdir(person_folder):
        ...
        if os.path.isdir(person_path):
            ...
            for image_file in os.listdir(person_path):
                ...
                current_ram_usage = torch.cuda.memory_allocated() if torch.cuda.is_available() else 0
                if current_ram_usage > 4e9:  # 4 GB
                    torch.cuda.empty_cache()

            if embeddings:
                embeddings_tensor = torch.cat(embeddings)
                save_path = os.path.join(save_folder, f"{person}_embeddings.pt")
                torch.save(embeddings_tensor, save_path)
                print(f"Embeddings saved for {person} to {save_path}")
            else:
